...
public class IpAccessPolicyService {
    ...
    private static final List<String> IMPLICIT_BYPASS_PREFIXES = List.of("/sys/ip-access-policy", "/sys/ip-access-rule");
    ...
    public void refresh() {
        List<SysTenant> tenants = this.sysTenantRepository.findAll().stream().filter(t -> t.getDeletedAt() == null).toList();
    ...
    private PolicySnapshot buildSnapshotForTenant(long tid) {
        ...
        List<String> bypass = IpAccessPolicyService.mergeBypassPrefixes((String)bypassRaw);
    ...
    private static List<String> mergeBypassPrefixes(String commaSeparatedFromDb) {
        List<String> fromDb = IpAccessPolicyService.parseCommaPrefixes((String)commaSeparatedFromDb);
    ...
    private static enum PolicyMode {
        OFF,
        BLACKLIST,
        WHITELIST;


        static IpAccessPolicyService.PolicyMode from(String s) {
    ...
    private record PolicySnapshot(PolicyMode mode, List<String> bypassPrefixes, List<String> denyCidrs, List<String> allowCidrs) {
        static IpAccessPolicyService.PolicySnapshot empty() {
            return new PolicySnapshot(PolicyMode.OFF, List.of(), List.of(), List.of());
        }
    }
}
