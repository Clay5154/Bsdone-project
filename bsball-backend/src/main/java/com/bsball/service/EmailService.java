...
public class EmailService {
    ...
    public void sendCode(String email, String scene, long tenantId, String clientIp) {
        ...
        this.emailCodeRepository.save(record);
        boolean sent = false;
        ...
        this.sendCooldown.put((normalized + ":" + scene), Boolean.TRUE);
    }
    ...
    private void deliver(String to, String code, int expireMin) throws Exception {
        ...
        Session session = Session.getInstance((Properties)props, new Authenticator() { protected PasswordAuthentication getPasswordAuthentication() { return new PasswordAuthentication(emailProperties.getUsername(), emailProperties.getPassword()); } });
    ...
    private void checkAntiAbuse(String email, String scene) {
        if (this.sendCooldown.getIfPresent((email + ":" + scene)) != null) {
