...
implements IpLocationProvider {
    ...
    public Optional<IpLocationDetail> locate(String ip) {
        ...
        try {
            ...
            if (!"1".equals(AmapIpLocationProvider.text((JsonNode)root, "status"))) {
                if (AmapIpLocationProvider.isQosLike((String)AmapIpLocationProvider.text((JsonNode)root, "info"))) {
            ...
            String prov = AmapIpLocationProvider.text((JsonNode)root, "province");
            String city = AmapIpLocationProvider.cityText((JsonNode)root);
            String adcode = AmapIpLocationProvider.text((JsonNode)root, "adcode");
            String rectStr = AmapIpLocationProvider.text((JsonNode)root, "rectangle");
