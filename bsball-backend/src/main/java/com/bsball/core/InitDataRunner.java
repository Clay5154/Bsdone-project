 ...
 *  com.bsball.core.InitDataRunner$PortalRole
...
implements CommandLineRunner {
    ...
    private void ensureAdminUserIfNeeded() {
        ...
        if (adminRole == null) {
            ...
            adminRole = (SysRole)this.sysRoleRepository.save(adminRole);
        ...
        if (adminRole != null) {
            ...
            this.sysRoleRepository.save(adminRole);
        }
        SysUserRole ur = new SysUserRole();
        ur.setUserId(admin.getId());
        ur.setRoleId(adminRole.getId());
        this.sysUserRoleRepository.save(ur);
    ...
    private void ensurePublicViewCountConfigIfNeeded() {
        ...
        this.sysConfigRepository.save(c);
    ...
    private void ensurePortalMonitorMenusAndApisIfNeeded() {
        ...
        List<ApiDef> apis = List.of(new ApiDef("/sys/portal/devtools-report/list", "GET", "\u67e5\u8be2\u95e8\u6237\u5f00\u53d1\u8005\u5de5\u5177\u6253\u5f00\u8bb0\u5f55", "\u7cfb\u7edf\u76d1\u63a7\u7ba1\u7406"), new ApiDef("/sys/portal/visit-hit/list", "GET", "\u67e5\u8be2\u95e8\u6237\u8bbf\u95ee\u6253\u70b9\u660e\u7ec6", "\u7cfb\u7edf\u76d1\u63a7\u7ba1\u7406"), new ApiDef("/sys/portal/ip-location", "GET", "IP \u5f52\u5c5e\u5730\uff08\u9ad8\u5fb7\uff09", "\u7cfb\u7edf\u76d1\u63a7\u7ba1\u7406"), new ApiDef("/sys/portal/ip-location/batch", "POST", "\u6279\u91cf IP \u5f52\u5c5e\u5730\uff08\u9ad8\u5fb7\uff09", "\u7cfb\u7edf\u76d1\u63a7\u7ba1\u7406"));
        ...
        for (ApiDef a : apis) {
            ...
            SysApi saved2 = (SysApi)this.sysApiRepository.save(api);
            existing.add(saved2);
        }
        ...
        if (!hasDevtools) {
            ...
            saved = (SysMenu)this.sysMenuRepository.save(m2);
            if (adminRole != null) {
    ...
    private void ensureIpAccessPolicyMenuAndApisIfNeeded() {
        ...
        for (ApiDef a : apis) {
            ...
            api.setCreatedBy(Long.valueOf(opId));
            api.setUpdatedBy(Long.valueOf(opId));
            SysApi saved = (SysApi)this.sysApiRepository.save(api);
            existing.add(saved);
        }
        ...
        SysMenu saved = (SysMenu)this.sysMenuRepository.save(m2);
        if (adminRole != null) {
            this.bindMenuToAdminIfNeeded(adminRole.getId().longValue(), saved.getId().longValue());
        }
    ...
    private void ensureIpLocationGeoMenuAndApisIfNeeded() {
        long opId = this.sysUserRepository.findByUsernameAndDeletedAtIsNull("admin").map(BaseEntity::getId).orElse(1L);
        List<ApiDef> apis = List.of(new ApiDef("/sys/ip-location-cache/page", "GET", "\u5206\u9875\u67e5\u8be2 IP \u5f52\u5c5e\u5730\u7f13\u5b58", "IP\u5730\u7406\u4fe1\u606f"), new ApiDef("/sys/ip-location-cache/lbs-providers", "GET", "\u67e5\u8be2\u53ef\u9009 IP \u5f52\u5c5e\u5730\u6570\u636e\u6e90", "IP\u5730\u7406\u4fe1\u606f"), new ApiDef("/sys/ip-location-cache/refresh", "POST", "\u5f3a\u5236\u91cd\u65b0\u89e3\u6790 IP \u5e76\u5199\u56de\u7f13\u5b58", "IP\u5730\u7406\u4fe1\u606f"));
        ArrayList<SysApi> existing = new ArrayList<SysApi>(this.sysApiRepository.findAll());
        for (ApiDef a : apis) {
            boolean exists = existing.stream().anyMatch(e -> a.path.equals(e.getPath()) && a.method.equalsIgnoreCase(e.getMethod()));
            if (exists) continue;
    ...
    private void reorderSystemMenusIpGeoAfterIpPolicy() {
        ArrayList<SysMenu> all = new ArrayList<>(this.sysMenuRepository.findAll());
        SysMenu systemRoot = all.stream().filter(m -> "/system".equals(m.getPath()) || "\u7cfb\u7edf\u7ba1\u7406".equals(m.getTitle())).findFirst().orElse(null);
        ...
        List<SysMenu> children = all.stream().filter(m -> systemRoot.getId().equals(m.getParentId())).sorted(Comparator.comparingInt(SysMenu::getSort).thenComparing(BaseEntity::getId)).collect(Collectors.toCollection(ArrayList::new));
        ...
        if (changed) {
            this.sysMenuRepository.saveAll(children);
    ...
    private void ensureLoginAndOperationLogsUnderMonitorIfNeeded() {
        ...
        for (SysMenu m2 : all) {
            ...
            if ("/admin/login-logs".equals(m2.getPath())) {
                ...
                if (changed) {
                    m2.setUpdatedBy(Long.valueOf(opId));
                    this.sysMenuRepository.save(m2);
    ...
    private void ensureTenantAdminRoleIfNeeded() {
        ...
        if (role == null) {
            ...
            role = (SysRole)this.sysRoleRepository.save(role);
            created = true;
        ...
        List<SysMenu> menus = this.sysMenuRepository.findAll();
        Set<Long> allowedMenuIds = InitDataRunner.collectTenantAdminDefaultMenuIds((List)menus);
        Set<Long> boundMenus = this.sysRoleMenuRepository.findByRoleId(role.getId()).stream().map(SysRoleMenu::getMenuId).collect(Collectors.toCollection(HashSet::new));
        ...
        Set<Long> boundApis = this.sysRoleApiRepository.findByRoleId(role.getId()).stream().map(SysRoleApi::getApiId).collect(Collectors.toCollection(HashSet::new));
        int added = 0;
    ...
    private static Set<Long> collectTenantAdminDefaultMenuIds(List<SysMenu> menus) {
        ...
        Set<String> leafPaths = Set.of("/admin/dashboard", "/admin/users", "/admin/roles", "/admin/config", "/admin/resources", "/admin/media-icons", "/admin/media-gallery", "/admin/announcements", "/admin/monitor/portal-devtools-report", "/admin/monitor/portal-visit-hit", "/admin/monitor/portal-feedback", "/admin/monitor/ip-access-policy", "/admin/login-logs", "/admin/operation-logs");
    ...
    private void ensurePortalRolesIfNeeded(long opId) {
        List<PortalRole> roles = List.of(new PortalRole("member", "\u666e\u901a\u7528\u6237", "\u95e8\u6237\u81ea\u52a9\u6ce8\u518c\u9ed8\u8ba4\u89d2\u8272\uff1a\u53ef\u8ba4\u9886\u7403\u5458\u3001\u7533\u8bf7\u52a0\u5165\u8054\u76df\u7403\u961f", 50), new PortalRole("team_manager", "\u7403\u961f\u8d1f\u8d23\u4eba", "\u542b\u666e\u901a\u7528\u6237\u6743\u9650\uff0c\u53ef\u521b\u5efa\u7403\u961f\u3001\u6dfb\u52a0\u7403\u5458\u3001\u5ba1\u6838\u7403\u5458\u8ba4\u9886", 51), new PortalRole("league_organizer", "\u8d5b\u4e8b\u4e3b\u529e\u65b9", "\u542b\u7403\u961f\u8d1f\u8d23\u4eba\u6743\u9650\uff0c\u53ef\u521b\u5efa\u8054\u76df\u3001\u7ba1\u7406\u672c\u8054\u76df\u8d5b\u4e8b\uff08\u4ec5\u9650\u672c\u8054\u76df\uff09", 52));
    ...
    private void ensureAdminRoleBindsAllApisIfNeeded() {
        ...
        Set<Long> bound = this.sysRoleApiRepository.findByRoleId(adminRole.getId()).stream().map(SysRoleApi::getApiId).collect(Collectors.toCollection(HashSet::new));
    ...
    private void ensureStadiumMenuIfNeeded() {
        ...
        SysMenu savedMenu = (SysMenu)this.sysMenuRepository.save(stadiumMenu);
        SysRole adminRole = this.sysRoleRepository.findByTenantIdIsNullAndCode("admin").orElse(null);
        if (adminRole != null && !(alreadyBound = this.sysRoleMenuRepository.findByRoleId(adminRole.getId()).stream().anyMatch(rm -> savedMenu.getId().equals(rm.getMenuId())))) {
            ...
            this.sysRoleMenuRepository.save(rm2);
            log.info("\u5df2\u8865\u63d2\u83dc\u5355\uff1a\u7403\u573a\u7ba1\u7406");
        }
    }
    ...
    private static List<Stadium> defaultStadiumSeeds() {
        ...
        list.add(InitDataRunner.stadium("\u4e2d\u5c71\u718a\u732b\u7eaa\u5ff5\u7403\u573a", "\u718a\u732b\u7eaa\u5ff5\u7403\u573a", "Panda Memorial Field", "\u5e7f\u4e1c\u7701", "\u4e2d\u5c71\u5e02", "\u5c0f\u6984\u9547", "442000118", "528415", "\u5c0f\u6984\u9547\u4e1c\u5347\u7247\u540c\u5fb7\u8857\uff09", (Point)InitDataRunner.gcjPoint((double)113.29682, (double)22.61505), (StadiumLevel)StadiumLevel.A, (int)3000, (int)1000, (int)100, (int)122, (int)100, (TurfType)TurfType.NATURAL));
        list.add(InitDataRunner.stadium("\u4e2d\u5c71\u4e1c\u5347\u4e2d\u5b66\u7403\u573a", "\u4e1c\u5347\u4e2d\u5b66\u7403\u573a", "Dongsheng HS Field", "\u5e7f\u4e1c\u7701", "\u4e2d\u5c71\u5e02", "\u5c0f\u6984\u9547", "442000118", "528414", "\u4e1c\u5347\u5927\u905328\u53f7\u4e2d\u5c71\u5e02\u4e1c\u5347\u9ad8\u7ea7\u4e2d\u5b66", (Point)InitDataRunner.gcjPoint((double)113.29095, (double)22.61038), (StadiumLevel)StadiumLevel.B, (int)800, (int)400, (int)97, (int)120, (int)97, (TurfType)TurfType.NATURAL));
        list.add(InitDataRunner.stadium("\u5e7f\u5dde\u5e7f\u4f53\u68d2\u7403\u573a", "\u5e7f\u4f53\u68d2\u7403\u573a", "GZSU Baseball Field", "\u5e7f\u4e1c\u7701", "\u5e7f\u5dde\u5e02", "\u5929\u6cb3\u533a", "440106", "510500", "\u5e7f\u5dde\u5927\u9053\u4e2d1268\u53f7\u5e7f\u5dde\u4f53\u80b2\u5b66\u9662\u4e3b\u6821\u533a\u6821\u5185\u68d2\u7403\u573a", (Point)InitDataRunner.gcjPoint((double)113.31805, (double)23.15218), (StadiumLevel)StadiumLevel.B, (int)1500, (int)800, (int)100, (int)122, (int)100, (TurfType)TurfType.NATURAL));
        list.add(InitDataRunner.stadium("\u6df1\u5733\u4e2d\u5c71\u516c\u56ed\u68d2\u7403\u573a", "\u4e2d\u5c71\u516c\u56ed\u68d2\u7403\u573a", "Zhongshan Park SZ", "\u5e7f\u4e1c\u7701", "\u6df1\u5733\u5e02", "\u5357\u5c71\u533a", "440305", "518052", "\u5357\u5c71\u5927\u90533109\u53f7\u6df1\u5733\u4e2d\u5c71\u516c\u56ed\u5185", (Point)InitDataRunner.gcjPoint((double)113.92655, (double)22.53612), (StadiumLevel)StadiumLevel.A, (int)2500, (int)1200, (int)100, (int)125, (int)100, (TurfType)TurfType.NATURAL));
        return list;
    }
    ...
    private void ensureArticleMenuIfNeeded() {
        ...
        SysMenu saved = (SysMenu)this.sysMenuRepository.save(articleMenu);
        SysRole adminRole = this.sysRoleRepository.findByTenantIdIsNullAndCode("admin").orElse(null);
        if (adminRole != null) {
            this.bindMenuToAdminIfNeeded(adminRole.getId().longValue(), saved.getId().longValue());
        }
    ...
    private void ensureHistoryRecordMenuIfNeeded() {
        ...
        if (existed != null) {
            ...
            if (changed) {
                this.sysMenuRepository.save(existed);
            }
            return;
        }
        SysMenu businessRoot = all.stream().filter(m -> "/business".equals(m.getPath()) || "\u4e1a\u52a1\u7ba1\u7406".equals(m.getTitle())).findFirst().orElse(null);
        ...
        SysMenu savedMenu = (SysMenu)this.sysMenuRepository.save(personnelMenu);
        SysRole adminRole = this.sysRoleRepository.findByTenantIdIsNullAndCode("admin").orElse(null);
        if (adminRole != null && !(alreadyBound = this.sysRoleMenuRepository.findByRoleId(adminRole.getId()).stream().anyMatch(rm -> savedMenu.getId().equals(rm.getMenuId())))) {
            SysRoleMenu rm2 = new SysRoleMenu();
            rm2.setRoleId(adminRole.getId());
    ...
    private void ensureHighlightMomentMenuIfNeeded() {
        ...
        SysMenu savedMenu = (SysMenu)this.sysMenuRepository.save(highlightMenu);
        SysRole adminRole = this.sysRoleRepository.findByTenantIdIsNullAndCode("admin").orElse(null);
        if (adminRole != null) {
    ...
    private void ensureLineupTemplateMenuIfNeeded() {
        ...
        SysMenu savedMenu = (SysMenu)this.sysMenuRepository.save(lineupMenu);
        SysRole adminRole = this.sysRoleRepository.findByTenantIdIsNullAndCode("admin").orElse(null);
        if (adminRole != null) {
            this.bindMenuToAdminIfNeeded(adminRole.getId().longValue(), savedMenu.getId().longValue());
        }
    ...
    private void ensurePlayerClaimMenuIfNeeded() {
        ...
        SysMenu savedMenu = (SysMenu)this.sysMenuRepository.save(claimMenu);
        SysRole adminRole = this.sysRoleRepository.findByTenantIdIsNullAndCode("admin").orElse(null);
        if (adminRole != null) {
            this.bindMenuToAdminIfNeeded(adminRole.getId().longValue(), savedMenu.getId().longValue());
        }
    ...
    private void ensureTenantManagementMenusIfNeeded() {
        ...
        m2 = (SysMenu)this.sysMenuRepository.save(m2);
        if (adminRole != null) {
            this.bindMenuToAdminIfNeeded(adminRole.getId().longValue(), m2.getId().longValue());
        }
        log.info("\u5df2\u8865\u63d2\u83dc\u5355\uff1a\u79df\u6237\u7ba1\u7406");
    ...
    private void ensureMediaIconGalleryMenusAndApisIfNeeded() {
        ...
        for (ApiDef a : apis) {
            ...
            api.setCreatedBy(Long.valueOf(opId));
            api.setUpdatedBy(Long.valueOf(opId));
            existingApis.add((SysApi)this.sysApiRepository.save(api));
        }
        List<SysMenu> allMenus = this.sysMenuRepository.findAll();
    ...
    private void seedMenus(long adminId) {
        SysMenu m1 = (SysMenu)this.sysMenuRepository.save(this.menu(Long.valueOf(0L), "\u5de5\u4f5c\u53f0", "\u5de5\u4f5c\u53f0", "/admin/dashboard", "AdminDashboard", "views/admin/Dashboard.vue", "Odometer", 0, adminId));
        SysMenu m2 = (SysMenu)this.sysMenuRepository.save(this.menu(Long.valueOf(0L), "\u7cfb\u7edf\u7ba1\u7406", "\u7cfb\u7edf\u7ba1\u7406", "/system", null, null, "Setting", 1, adminId));
        SysMenu m4 = (SysMenu)this.sysMenuRepository.save(this.menu(Long.valueOf(0L), "\u7cfb\u7edf\u76d1\u63a7", "\u7cfb\u7edf\u76d1\u63a7", "/monitor", null, null, "Monitor", 2, adminId));
        SysMenu m3 = (SysMenu)this.sysMenuRepository.save(this.menu(Long.valueOf(0L), "\u4e1a\u52a1\u7ba1\u7406", "\u4e1a\u52a1\u7ba1\u7406", "/business", null, null, "Trophy", 3, adminId));
        int ms = 0;
    ...
    private void ensureMenuDirectoryTypesAndDefaultButtons() {
        ...
        if (changed) {
            this.sysMenuRepository.saveAll(all);
        ...
        for (ButtonSeed s : seeds) {
            ...
            b2 = (SysMenu)this.sysMenuRepository.save(b2);
            ++inserted;
            ...
            for (String bind : s.apiBindings) {
                ...
                this.sysMenuApiRepository.save(link);
            }
        }
        if (inserted > 0) {
    ...
    private Long resolveApiId(String methodPath, Map<String, Long> directLookup) {
        ...
        for (SysApi a : this.sysApiRepository.findAll()) {
            if (a.getMethod() == null || a.getPath() == null || !a.getMethod().trim().equalsIgnoreCase(method) || !InitDataRunner.pathTemplateMatches(a.getPath(), (String)template)) continue;
            return a.getId();
        }
        return null;
    }
    ...
    private void ensureAdminRoleBindsAllMenusIfNeeded() {
        ...
        Set<Long> bound = this.sysRoleMenuRepository.findByRoleId(adminRole.getId()).stream().map(SysRoleMenu::getMenuId).collect(Collectors.toCollection(HashSet::new));
        long opUserId = this.sysUserRepository.findByUsernameAndDeletedAtIsNull("admin").map(BaseEntity::getId).orElse(1L);
        int added = 0;
    ...
    private void seedDict(long adminId) {
        ...
        if (this.sysDictTypeRepository.findAllByType("sys_user_sex").isEmpty()) {
            ...
            dt = (SysDictType)this.sysDictTypeRepository.save(dt);
            for (String[] e : List.of(new String[]{"\u672a\u77e5", "0"}, new String[]{"\u7537", "1"}, new String[]{"\u5973", "2"})) {
                dd = new SysDictData();
                ...
                this.sysDictDataRepository.save(dd);
            }
        }
        if (this.sysDictTypeRepository.findAllByType("api_group").isEmpty()) {
            ...
            apiGroup = (SysDictType)this.sysDictTypeRepository.save(apiGroup);
            int sort = 0;
            for (String g : List.of("\u8ba4\u8bc1", "\u7528\u6237", "\u89d2\u8272", "\u83dc\u5355", "API\u7ba1\u7406", "\u5b57\u5178\u7c7b\u578b", "\u5b57\u5178\u6570\u636e", "\u5b57\u5178", "\u6587\u7ae0\u7ba1\u7406", "\u901a\u77e5\u7ba1\u7406", "\u8d44\u6e90", "\u56fe\u6807\u7ba1\u7406", "\u56fe\u5e93\u7ba1\u7406", "\u767b\u5f55\u5386\u53f2", "\u64cd\u4f5c\u5386\u53f2", "\u914d\u7f6e", "\u7cfb\u7edf\u76d1\u63a7", "IP\u5730\u7406\u4fe1\u606f", "\u8054\u76df", "\u7403\u961f", "\u6559\u7ec3\u7ba1\u7406", "\u8d5b\u4e8b", "\u6bd4\u8d5b", "\u6570\u636e\u6392\u884c", "\u7403\u5458", "\u5185\u5bb9\u7ba1\u7406", "\u4eba\u4e8b\u53d8\u52a8\u7ba1\u7406", "\u7403\u573a\u7ba1\u7406", "\u95e8\u6237\u7edf\u8ba1")) {
                ...
                this.sysDictDataRepository.save(dd2);
            }
        }
        if (this.sysDictTypeRepository.findAllByType("venue").isEmpty()) {
            ...
            venue = (SysDictType)this.sysDictTypeRepository.save(venue);
            for (String[] e : List.of(new String[]{"\u4e3b\u573a", "home"}, new String[]{"\u5ba2\u573a", "away"}, new String[]{"\u4e2d\u7acb", "neutral"})) {
                dd = new SysDictData();
        ...
        if (this.sysDictTypeRepository.findAllByType("player_status").isEmpty()) {
            ...
            playerStatus = (SysDictType)this.sysDictTypeRepository.save(playerStatus);
            for (String[] e : List.of(new String[]{"\u5728\u5f79", "active"}, new String[]{"\u9000\u5f79", "retired"}, new String[]{"\u4f24\u75c5", "injured"}, new String[]{"\u5176\u4ed6", "other"})) {
                dd = new SysDictData();
        ...
        if (this.sysDictTypeRepository.findAllByType("notice_type").isEmpty()) {
            ...
            noticeType = (SysDictType)this.sysDictTypeRepository.save(noticeType);
            for (String[] e : List.of(new String[]{"\u8d44\u8baf", "news"}, new String[]{"\u516c\u544a", "announcement"}, new String[]{"\u901a\u77e5", "notice"}, new String[]{"\u516c\u793a", "publicity"})) {
                dd = new SysDictData();
        ...
        if (this.sysDictTypeRepository.findAllByType("content_format").isEmpty()) {
            ...
            contentFormat = (SysDictType)this.sysDictTypeRepository.save(contentFormat);
            for (String[] e : List.of(new String[]{"HTML", "html"}, new String[]{"Markdown", "markdown"})) {
                dd = new SysDictData();
    ...
    private void seedConfig(long adminId) {
        for (String[] e : List.of(new String[]{"siteName", "BS Ball"}, new String[]{"siteTitle", "\u8d5b\u4e8b\u4e0e\u6570\u636e\u5c55\u793a"}, new String[]{"publicViewCount", "true"}, new String[]{"portalDevtoolsGuard", "true"}, new String[]{"portalDevtoolsGuardOverlay", "true"}, new String[]{"portalDevtoolsGuardDebuggerTrap", "true"}, new String[]{"portalDevtoolsGuardCopyrightNotice", "true"})) {
            SysConfig c = new SysConfig();
    ...
    private void seedAdminRoleAndBindUser(long adminId) {
        List<SysMenu> menus = this.sysMenuRepository.findAll();
        List<SysApi> apis = this.sysApiRepository.findAll();
        List<Long> allMenuIds = menus.stream().map(BaseEntity::getId).collect(Collectors.toList());
        List<Long> allApiIds = apis.stream().map(BaseEntity::getId).collect(Collectors.toList());
        List<Long> guestApiIds = apis.stream().filter(arg_0 -> this.isGuestRolePublicGetApi(arg_0)).map(BaseEntity::getId).collect(Collectors.toList());
        SysRole adminRole = this.sysRoleRepository.findByTenantIdIsNullAndCode("admin").orElse(null);
        if (adminRole == null) {
            adminRole = new SysRole();
            adminRole.setName("\u8d85\u7ea7\u7ba1\u7406\u5458");
        ...
        List<Long> guestMenuIds = menus.stream().filter(m -> m.getPath() != null && guestMenuPaths.contains(m.getPath())).map(BaseEntity::getId).distinct().toList();
        ...
        guestRole = (SysRole)this.sysRoleRepository.save(guestRole);
        ...
        if (this.sysUserRepository.count() == 0L) {
            SysUser admin = (SysUser)this.sysUserRepository.findById(adminId).orElseThrow();
            SysUserRole ur = new SysUserRole();
            ur.setUserId(admin.getId());
    ...
    record PortalRole(String code, String name, String desc, int sort) {
    }

    private record ApiDef(String path, String method, String desc, String group) {
    }

    private record ButtonSeed(String parentPath, String title, String permission, int sort, String[] apiBindings) {
    }
}
